# -*- coding: utf-8 -*-
"""A collection of matrix and vector translation tools

These tools can be used to translate a numpy/pandas matrix/vector from one
format to another when provided a set of weights.
These functions will optimise for speed (by using more memory) when possible,
and revert to a slower method when not possible.
"""
...
def _lower_memory_matrix_zone_translation(
    ...
    """Translate a matrix between two zoning systems

    Internal function of `numpy_matrix_zone_translation`.
    This is a much slower method of translation (even when multi-processed),
    but uses much less memory too.
    """
    # Translate the rows
    kwargs = list()
    ...
    row_translated_ary = np.vstack(row_translated).T

    # Translate the rows
    kwargs = list()
    n_splits = row_translated_ary.shape[0] / chunk_size
    n_splits = 1 if n_splits <= 0 else n_splits
    for vector_chunk in np.array_split(row_translated_ary, n_splits, axis=0):  # type: ignore
        kwargs.append(
            {"vector_chunk": vector_chunk, "col_translation": col_translation}
        )
...
def numpy_matrix_zone_translation(
    ...
    row_translation: np.ndarray = None,
    ...
    process_count:
        If there isn't enough memory for a three-dimensional translation, this
        number of processes will be used for a slower, multi-processed
        translation process.
    ...
    if mat_max > dtype_max:
        raise ValueError(
            f"Maximum value in matrix is greater than the given "
            f"translation_dtype can handle.\n"
            f"Maximum dtype value: {dtype_max}\n"
            f"Maximum matrix value: {mat_max}")

    if mat_min < dtype_min:
        raise ValueError(
            f"Minimum value in matrix is less than the given "
            f"translation_dtype can handle.\n"
            f"Minimum dtype value: {dtype_min}\n"
            f"Minimum matrix value: {mat_min}"
        )
...
def numpy_vector_zone_translation(
    ...
    if mat_max > dtype_max:
        raise ValueError(
            f"Maximum value in vector is greater than the given "
            f"translation_dtype can handle.\n"
            f"Maximum dtype value: {dtype_max}\n"
            f"Maximum vector value: {mat_max}"
        )

    if mat_min < dtype_min:
        raise ValueError(
            f"Minimum value in vector is less than the given "
            f"translation_dtype can handle.\n"
            f"Minimum dtype value: {dtype_min}\n"
            f"Minimum vector value: {mat_min}"
        )
...
def pandas_matrix_zone_translation(
    ...
    if matrix.columns.dtype != matrix.index.dtype:
        raise ValueError(
            f"The datatype of the index and columns in matrix must be the"
            f"same for the zone translation to work.\n"
            f"Index Dtype: {matrix.index.dtype}\n"
            f"Column Dtype: {matrix.columns.dtype}"
        )
    ...
    if matrix.index.dtype != row_translation[from_zone_col].dtype:
        raise ValueError(
            ...
            "translation to work.\n"
            f"matrix index Dtype: {matrix.index.dtype}\n"
            f"row_translation[from_zone_col] Dtype: {row_translation[from_zone_col].dtype}"
        )
    ...
    if matrix.index.dtype != col_translation[from_zone_col].dtype:
        raise ValueError(
            ...
            "translation to work.\n"
            f"matrix index Dtype: {matrix.index.dtype}\n"
            f"col_translation[from_zone_col] Dtype: {col_translation[from_zone_col].dtype}"
        )
    ...
    if len(missing_cols) > 0:
        warnings.warn(
            ...
            "from_unique_zones. These zones will be dropped before the "
            "translation!"
            f"Additional cols count: f{len(missing_cols)}"
        )
    ...
    if len(missing_zones) != 0:
        warnings.warn(
            ...
            f"Missing zones count: f{len(missing_zones)}"
        )
...
def pandas_vector_zone_translation(
    ...
    if vector.index.dtype != translation[from_zone_col].dtype:
        raise ValueError(
            ...
            f"translation datatype in from_zone_col for the zone translation "
            f"to work.\n"
            f"vector index Dtype: {vector.index.dtype}\n"
            f"translation[from_zone_col] Dtype: {translation[from_zone_col].dtype}"
        )
